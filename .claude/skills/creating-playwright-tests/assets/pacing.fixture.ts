...
/**
 * TEMPLATE — copy into the target's test-scripts/ folder when config.yaml sets
 * a `pacing` block, then import `test`/`expect` from here instead of from
 * '@playwright/test':
 *
...
// <-- set from config.yaml pacing.min_seconds_between_navigations (in ms)
